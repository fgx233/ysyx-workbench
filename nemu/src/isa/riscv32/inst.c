...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  /* 逻辑运算 */
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and_   , R, R(rd) = src1 & src2);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or_    , R, R(rd) = src1 | src2);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(rd) = src1 | imm);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor_   , R, R(rd) = src1 ^ src2);
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (src2 == 0)? 0xFFFFFFFF : 
                                                                        (src1 == 0x80000000 && src2 == 0xFFFFFFFF)? src1 : ((sword_t)src1 / (sword_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (src2 == 0)? 0xFFFFFFFF : (src1 / src2));
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (src2 == 0)? src1 :
                                                                        (src1 == 0x80000000 && src2 == 0xFFFFFFFF)? 0 : (sword_t)src1 % (sword_t)src2);
